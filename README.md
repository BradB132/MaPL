...
MaPL is implemented in C and C++, and so should be relatively portable. However, I've only tested it on macOS. `MaPLCompiler` and `MaPLGenerator` currently have cmake files, while `MaPLRuntime` does not, because it's intended to only be run within a larger program. `MaPLGenerator` is dependent on `libxml2`.
